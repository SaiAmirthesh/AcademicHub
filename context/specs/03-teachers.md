...
- `image`: URL string pointing to Cloudinary-hosted profile avatar.
...
- **`POST /api/teachers`**: Creates a new teacher profile. Uploads profile image to Cloudinary beforehand. Restricted to **Admin** role.
...
- Image uploads are routed directly from the frontend to Cloudinary using unsigned upload presets (`VITE_CLOUDINARY_UPLOAD_PRESET`).
- The frontend uploads the file and obtains a `secure_url`, which is sent to the backend `POST/PUT /api/teachers` endpoints.

## 4. Frontend Refine UI
- **List View (`src/pages/teachers/list.tsx`)**: Table displaying Avatar (Cloudinary image render), Full Name, Email, Department, and Assigned Classes.
- **Create Form**: Form component containing a file dropzone. Uploads the image to Cloudinary first, then submits Name, Email, Department, and Avatar URL to the backend.
